...
- **Harbor town** (`town.tscn`, `town_probe.tscn`): the Maine coast with a 1940s town (`harbor_town.gd`, `TownMesh`); Water Street's harbour row modelled inside and out (`house_base.gd`, `cape_house.gd`, `full_house.gd`), boats, a bell buoy (`harbor.gd`), leafed trees (`TreeKit`), weather (`weather.gd`).
