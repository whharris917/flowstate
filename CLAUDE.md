...
- **Bench chemistry is its own layer.** `game/data/chemistry.json` is the hand-authored library of substances, reactions, acid families and the hold's reagent stocks, read by both kernels (`sim/chemistry.py`, `ChemLibrary`). A bench vessel's `SimMixture` counts moles of real substances, liquid and solid, on a mole basis with real densities; the plant's streams keep their six abstract species until the director decides to join the two. Rate laws are Arrhenius from k at 25 C; reversible reactions stop at K; pH is a charge balance; solubility is log-linear in temperature and in the solvent's water fraction; boiling is Raoult with Clausius-Clapeyron. A species' notes are prose drafts for the director. `BenchSelfCheck` holds the GD kernel to the numbers `tests/test_bench.py` holds the Python one to.
...
- Bench things (`BenchLayout`) stand where aimed on any surface on a 2 cm grid; a vessel aimed at a hotplate or balance sits centred on it. What stands on what (hotplate load, balance load, the meter's electrode) is derived from positions by `BenchLayout.sync`, on place, move, remove and at the end of a load. A vessel's device menu has a POUR tab (`Plant.pour`); an unstirred source decants, stock bottles are poured from and never into.
...
- Build palette: Tab opens the page rail and cards; B enters or leaves build mode; number keys pick from a nine-slot hotbar the player assigns (saved in `user://settings.json`). A page holds at most 9 entries; adding a page means changing `_page_catalog()`, `_is_equipment_page()`, `page_names` and `PAGES` in `build_controller.gd` and `PAGE_NAMES` in `build_palette.gd` together. Ten pages; the tenth is BENCH.
...
- **Solid to the player.** A new view needs nothing for this: whatever it draws becomes solid. A primitive's hull comes from its dimensions; a hand-built ArrayMesh is read back from the GPU (a millisecond or more), so keep those few.
...
- **Maine coast** (`world/maine.tscn`): the blank map's rules on a landscape (`landscape.gd`, `maine_coast.gd`) with a river and the heighliner as scenery. It hosts the exercises: routing, the drip demo (small-bore devices on tubing, on the home pad), line pressure gauges, the vial filling line built from parts (`world/fill_line_demo.gd`, 40 vials a minute), and the bench east of the home pad (`world/bench_demo.gd`: eleven reagents, a hotplate warming a beaker, a meter in a beaker of acid and indicator, a balance, a flask of bicarbonate).
- **Harbor town** (`town.tscn`, `town_probe.tscn`): the Maine coast with a 1940s town on a hillside (`harbor_town.gd`); Water Street's harbour row modelled inside and out (`full_house.gd`); Main Street after Montpelier (`main_street.gd`); boats and a bell buoy (`harbor.gd`), trees (`TreeKit`), weather (`weather.gd`).
...
**What exists:** both kernels on pressure with species streams; the control tier (PLC with ladder editor, PID, valves, relays, cabinets built module by module (the `pd8` module is a fused 24 V distribution strip, `PowerDistribution` in both kernels), junction boxes and multicores, stations); the process train (boiler, exchanger, reactor, crystallizer, centrifuge, dryer, still, vacuum lock, vial filler); small-bore family; filling-line parts; alarms; trend screens; the material balance screen; save/load; undo; graphics presets; bench chemistry (a library of 32 substances, 16 reactions and 26 reagent stocks; beakers, flasks, sample vials and reagent bottles; a hotplate stirrer, a pH and temperature meter, a balance; pouring; the library panel's SUBSTANCES and REACTIONS pages; unlocked by the first milestone); the player's body (`player/player_figure.gd`, posed from movement, footsteps on foot strikes) and a 0.6 m jump on Space. Equipment, lines and cables are solid to the player (`world/plant_solids.gd`, layer 32, which only the player and its camera see): equipment as one convex hull per drawn primitive (MeshMerge keeps the pieces as the merged mesh's `sources`), lines as capsules along their drawn path; rebuilt when a mesh is added or removed under a thing; nothing while carried.

**Showcase routing counts:** 0 crossings, 0 overlaps, 0 unsupported, 17 through solids (layout debt; two are Unit 400 tray tails crossing a deck railing beside their switch). Maine: all 0.
...
**Cable plan** (director, 2026-09-24), in order: (1) loose cables, done; (2) sleeves, done: `run_sleeve` on the routing page, laid on the floor by clicks; T on a loose cable threads it into the sleeve whose ends are nearest its terminals (within 8 m), T again takes it out (`Plant.toggle_sleeve`, `_cable_carriers`, saved as the wire's `sleeve`); the sleeve grows with its cables; a sleeve can be clear (E on it: tinted glass, the cables seen inside); the filling room's field cables run in SL-601, clear; the Unit 400 multicore is still its own thing; (3) trays, begun: T threads a cable into a tray as into a sleeve (`Plant._trayed_route`): in at the tray point nearest its source, on the tray floor in a slot of its own, out nearest its destination; trays 400 and 150 mm, cables side by side then in layers; the filling room's 24 V leads run in CT-601. Conduit (`run_conduit`) takes cables end to end like a sleeve, the cable held to its bends (`_tight_corners`); the drip demo's two cables run in CD-201. Trays and conduit refuse a cable past 40 % fill (`_carrier_capacity`); the Maine smoke prints each carrier's fill. A sleeve goes into a tray or conduit with T (`_toggle_sleeve_carrier`, carrier key `sleeve:<name>`): its ends stay, the carrier holds its middle, and its cables count toward the fill. A tray's cable tails are routed round obstacles, in the air (`_floor_corners(..., on_floor = false)`); on a riser a cable comes in round the side rail (`_tray_way_in`). Trays and conduit are reshaped corner by corner like a sleeve (`_placed_of_view`, meta `editable_run`). Out of a conduit's mouth a cable runs 25 cm straight before it bends (`CONDUIT_MOUTH`). Cables between floors: Unit 400's deck circuits run in CT-401, a vertical 150 mm tray at the tower's north-east corner. A cable's route search uses a probe 0.26 m wide (`ROUTE_PROBE`), retried at the cable's own size if boxed in; RunClearance samples a cable every 5 cm.

**First things next session:**
0. **The bench, open for the director:** the library of substances, reactions and stocks and its notes (drafts, in `chemistry.json`), seven library page drafts, the Maine bench, and whether the bench and the plant's six species should become one chemistry.
1. **Filling line, open for the director:** the compact room (16 x 8 m, the cabinet and its one feeder 3 m from the line, 24 V from the cabinet's fused strip through tray CT-601, field cables in clear sleeve SL-601), ten library page drafts (the nine line parts and the 24 V distribution strip), the campaign rung. Not done: 480 V feeders into trays (feeders stay pipe-routed conduit runs).
