...
The control tier is in (kernel-first, Python `sim/control.py` + GD mirrors, 68 pytest): PLC (di/do/ai/ao channels, memories, TON timers, scanned validated ladder program), PID (anti-windup, bumpless A/M), control valve, terminal blocks, mains feeder / PSU with POWER port kind + voltage-class specs (mismatches refused; pumps/reboiler/PLC genuinely dead unpowered). Control cabinets are composites (PLC + PSU + 4 interposing relays + 10 terminals sharing one enclosure view; port markers keyed record:port): field wires land on flank markers, internal hookup happens in the 2D schematic panel (hidden kernel wires, live-lit), and the ladder editor (from the cabinet panel) edits the program with live power flow. Right-click any device for its I/O menu (declared ports, kinds, voltage specs, live values) to start/finish routed connections. Headless gauntlet is now four checks: kernel, control (PID + ladder), build-api (incl. cabinet field->PLC->relay->field traversal and unpowered-rack refusal), support. Still to come: panel-UI/HMI as placeable, more field devices (buttons, lights, transmitters as distinct devices, motorized valves), load accounting/breakers, P&ID view. Engine choice was re-confirmed with the director on 2026-08-29 (Unity considered, Godot chosen).
