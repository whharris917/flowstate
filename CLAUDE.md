...
- **Harbor town** (`world/town.tscn`; `town_probe.tscn`): the Maine coast with a 1940s town (`harbor_town.gd`, `TownMesh`), a Cape modelled inside and out (`cape_house.gd`), boats and a bell buoy (`harbor.gd`), leafed trees (`TreeKit`), weather from one level (`weather.gd`). Its own clock and weather.
