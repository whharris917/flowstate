...
const PAGES := {
	"tank": {
		"title": "Storage Tank",
		"tier": "process",
		"summary": "Holds liquid, and knows what the liquid is. Anything arriving blends into the contents, so a hot stream genuinely warms the vessel and a reagent charge genuinely changes what is in it. What leaves does so at whatever the contents currently are. Overfill it and it spills, and the spill is counted.\n\nIts two nozzles differ only in where they stand on the shell, and that is the whole of its hydraulic behaviour. A nozzle under the liquid carries the head of everything standing above it; one above the liquid sits at headspace pressure and passes nothing out. Which is why a full tank will drain into an empty one through nothing but a pipe, why filling one through a top nozzle never has to fight its own level, and why a vessel keeps a heel below its outlet: weld the outlet where you want the heel. A nozzle takes the size of the line on it.\n\nA tank can be OPEN-TOPPED (the CONFIGURE tab): no roof, its liquid seen from above, and a line ending in the air over it lands what it delivers in it.",
		"ports": {
			"inlet": "A nozzle, welded high on the shell by default. Above the liquid it cannot flow backwards; once the level is over it, it carries head like any other.",
			"outlet": "A nozzle, welded low on the shell by default. Under the liquid it carries the static head; material goes whichever way the network solves, and charging a vessel up through it is normal. It stops passing anything out once the level has fallen past it.",
		...
		"equations": [
			["P_nozzle = P_headspace + rho*g*(z + max(depth, h))", "The boundary pressure the network sees at a nozzle standing h above the base. Piezometric, so a submerged nozzle reads the same at any height, and elevation costs head without the solver ever learning what elevation is."],
			["Cv_nozzle = nozzle_cv_lps * (DN / 50)^2", "A nozzle takes the size of the line on it, and passes flow in proportion to its bore's area."],
			...
			["submergence = clamp((depth - h) / 0.03 m, 0, 1)", "A nozzle uncovers as the level falls past it, so a vessel tails off instead of siphoning itself dry. Smooth, so it does not chatter shut. Inflow is never limited by it."],
		],
		"params": [
			...
			["height_m", "m", "0", "Shell height. Sets capacity with diameter. A nozzle keeps its place on the shell as a fraction of it, so a taller vessel lifts its nozzles."],
			["diameter_m", "m", "0", "Shell diameter."],
			["headspace_kpa", "kPa", "0", "Blanket pressure over the liquid. Adds to both nozzles equally."],
			["nozzle_cv_lps", "L/s", "20", "What a DN50 nozzle passes wide open across a 1 bar drop; a nozzle of another size scales with its bore's area. The default suits a few litres a second; a line carrying tens needs a bigger vessel nozzle as much as a bigger pipe."],
			["nozzle_h_m", "m", "weld", "Where each nozzle stands on the shell, above the base: where you welded it (right-hold a nozzle to move it)."],
			["nozzle_dn", "DN", "line", "Each nozzle's nominal size: the size of the line on it, DN50 with none."],
		...
		"assumptions": [
			...
			"A nozzle is a point at a height. It has no diameter of its own against the level: the 3 cm it uncovers over stands in for that.",
		],
	},
	"pump": {
