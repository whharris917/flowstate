...
func _grab_point() -> Vector3:
	...
	# A sleeve, a tray or a conduit has no fittings: every straight of it
	# takes a corner. A cable lies on a floor that need not be level.
	var sleeve := _edit_run.has_meta("editable_run")
	if leg < (0 if sleeve else 1) or leg > path.size() - (2 if sleeve else 3):
		return Vector3.INF
	_grab_leg = leg
	var a: Vector3 = path[leg]
	var b: Vector3 = path[leg + 1]
	if _edit_run.style() != "cable" and not sleeve and absf(a.y - b.y) > 0.001:
		return Vector3.INF
