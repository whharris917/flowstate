...
## eye level in front of the face (zoomed in), knee at the eyes, shaft
## rising up and slightly behind (zoomed out to third person, still
## centered on the player). Yaw turns the body, pitch free-looks from
## whatever vantage point the wheel has chosen.
...
# The zoom track, eye-local: a quadratic through tip -> knee -> top so
# there is no kink where the blade meets the shaft.
...
var zoom_t: float = 0.0        # 0 = blade tip (first person), 1 = top of shaft
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		...
		zoom_t = clampf(zoom_t - ZOOM_STEP, 0.0, 1.0)
