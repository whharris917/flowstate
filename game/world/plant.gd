		...
		"tb4a":
			for i in range(4):
				records.append(_add_cab_record(cab, "terminal", "%s_t%d" % [base, i + 1],
					{"kind": "analog"}))
		"pd8":
			records.append(_add_cab_record(cab, "power_dist", base, {"ways": 8}))
...
func cabinet_field_records(cab: String) -> Array[String]:
	...
	for module_v: Variant in (cabinets.get(cab, {}) as Dictionary).get("modules", []):
		var module := module_v as Dictionary
		if str(module["type"]) in ["tb8d", "tb4a", "psu", "pd8"]:
			for record_name: String in module["records"]:
	...
	for module_v: Variant in entry["modules"]:
		...
		for record_name: String in module["records"]:
			...
			if record is SimTerminal:
				...
				y -= 0.115
			elif record is SimPowerDistribution:
				# Its supply in on the field-in flank; a row per way on the
				# field-out flank, down the column and on into the next.
				var anchors := {"in": Vector3(-0.72, y, z)}
				for i in (record as SimPowerDistribution).ways:
					if y < 0.25:
						y = 1.72
						z += 0.16
					anchors["way%d" % (i + 1)] = Vector3(0.72, y, z)
					y -= 0.115
				PlantFactory.attach_port_markers(view, record, "power_dist", anchors)
...
func _crossing_targets(skip: Dictionary, ignore: Array, order: int) -> Array:
	...
	for name_: String in runs:
		...
		if (entry["node"] as PipeView).style() in ["tray", "cable"]:
			continue  # a conduit lands on a tray, it does not hop it; a sleeve is on the floor
...
func intersection_report() -> PackedStringArray:
	...
	for name_: String in runs:
		var entry: Dictionary = runs[name_]
		var run_view := entry["node"] as PipeView
		var laid: Array[Vector3] = run_view.path() if run_view.style() == "cable" \
			else PipeRoute.lay(entry["points"])
		# A multicore lands on its junction box and its cabinet: every
		# record of those two is open to it.
		var open: Array = []
		var circuits: Array = entry.get("circuits", [])
		if not circuits.is_empty():
			var groups := [_approach_group(str((circuits[0] as Array)[0])),
				_approach_group(str((circuits[0] as Array)[2]))]
			for record_name: String in views:
				if groups.has(_approach_group(record_name)):
					open.append(record_name)
		if laid.size() >= 2:
			checks.append([laid, run_view.radius(), name_, [], open])
...
func place_run(kind: String, name_: String, sparse_local: Array, getter: Callable = Callable()) -> bool:
	...
	if runs.has(name_) or not StructureFactory.RUNS.has(kind):
		return false
	var points := sparse_local
	if kind == "run_sleeve":
		# A sleeve lies on the floor under the points it was laid by.
		points = []
		for p: Vector3 in sparse_local:
			var y := _floor_y(p)
			points.append(Vector3(p.x, y, p.z) if not is_nan(y) else p)
	runs[name_] = {"kind": kind, "node": null, "points": points, "color": "", "label": "",
		"getter": getter}
...
## A standalone run's view, from its entry. A sleeve is drawn like a
## loose cable at the size of what it carries, on the runs' layer, not
## the structure's: it carries nothing and the player walks over it.
func _build_run_view(name_: String) -> void:
	var entry: Dictionary = runs[name_]
	var spec: Dictionary = StructureFactory.RUNS[entry["kind"]]
	if entry["node"] != null:
		(entry["node"] as Node).queue_free()
	...
	if entry["kind"] == "run_sleeve":
		...
		view.set_meta("editable_run", name_)
		var clear := bool(entry.get("clear", false))
		view.setup(CableDrape.curve(_sleeve_skeleton(name_)), live,
			SLEEVE_CLEAR if clear else spec["color"], radius, name_, "cable", 8)
		view.config_cb = _configure_run
		entry["node"] = view
		view.service_label = str(entry.get("label", ""))
		return
	else:
	...
	view.config_cb = _configure_run
	entry["node"] = view
	if entry["kind"] in ["run_tray", "run_tray_150", "run_conduit"]:
		view.set_meta("editable_run", name_)
	if str(entry.get("color", "")) != "":
		view.apply_service(Color.html(str(entry["color"])), str(entry.get("label", "")))
...
func _configure_run(view: PipeView) -> void:
	...
	if sleeve != "":
		...
		config_panel.open_for_sleeve(bool(entry.get("clear", false)), str(entry.get("label", "")),
			func(clear: bool, label_: String) -> void: set_sleeve_look(sleeve, clear, label_))
		return
	var is_wire := false
	for visual in _wire_visuals:
		if visual["node"] == view:
...
func remove_run(view: PipeView) -> bool:
	...
	for visual in _wire_visuals:
		if visual["node"] == view:
			...
			if sleeve != "":
				_cable_carriers.erase(_cable_key(visual))
...
func set_wire_corners(view: PipeView, corners: Array) -> PipeView:
	checkpoint()
	var sleeve := _placed_of_view(view)
	if sleeve != "":
		# A sleeve's corners are its centre: its points are under it. A
		# tray's or a conduit's corners are its points.
		var radius := _sleeve_radius(sleeve) if _sleeve_of_view(view) != "" else 0.0
		var points: Array = []
		for corner: Vector3 in corners:
			points.append(corner - Vector3.UP * radius)
		var entry: Dictionary = runs[sleeve]
		entry["points"] = points
		entry.erase("laid")   # the path the lanes compare against, laid again when next asked
		if _sleeve_of_view(view) == "":
			_build_run_view(sleeve)
		var kept: Array = []
...
func delete_wire_corner(view: PipeView, point: Vector3) -> String:
	...
	if sleeve != "":
		...
		if corners.size() <= 2:
			return "it keeps its two ends: X removes it"
...
func wire_own_path(view: PipeView) -> Array[Vector3]:
	...
	if sleeve != "":
		...
		return PipeRoute.lay((runs[sleeve] as Dictionary)["points"])
	for visual in _wire_visuals:
		if visual["node"] == view:
...
func _revalidate_supports() -> void:
	...
	for name_: String in runs:
		var entry: Dictionary = runs[name_]
		if (entry["node"] as PipeView).style() == "cable":
			continue  # a sleeve lies on the floor
...
func _lay_route(src_name: String, src_port: String, dst_name: String, dst_port: String,
	...
	if _is_cable_run(src_name, src_port):
		...
		if sleeve != "" and runs.has(sleeve):
			if _is_tray(sleeve):
				return _trayed_route(src_name, src_port, dst_name, dst_port, radius, sleeve)
...
func _cable_route(src_name: String, src_port: String, dst_name: String, dst_port: String,
	...
	if not corners.is_empty():
		...
		if is_nan(floor_a) or is_nan(floor_b) or absf(floor_a - floor_b) >= 0.3:
			...
			floor_b = NAN
	var skeleton := _clear_drape(from, from_dir, to, to_dir, corners, floor_a, floor_b, radius, [src_name, dst_name])
...
func _cable_corners(src_name: String, src_port: String, dst_name: String, dst_port: String,
	...
	return _floor_corners(_marker_pos(src_name, src_port), _marker_dir(src_name, src_port),
		_marker_pos(dst_name, dst_port), _marker_dir(dst_name, dst_port), [src_name, dst_name],
		waypoints, radius)
...
## A cable's drape that clears what is beside its terminals: dropping
## out the way each terminal faces, unless that passes through
## something, then toward its next point.
func _clear_drape(from: Vector3, from_dir: Vector3, to: Vector3, to_dir: Vector3, corners: Array,
		floor_a: float, floor_b: float, radius: float, own: Array,
		straight_a: float = CableDrape.GLAND, straight_b: float = CableDrape.GLAND) -> Array[Vector3]:
	var facing := CableDrape.skeleton(from, from_dir, to, to_dir, corners, floor_a, floor_b, radius, true,
		straight_a, straight_b)
	var ctx := clearance.context(own, from, to, radius)
	if clearance.hits(CableDrape.curve(facing), ctx).is_empty():
		return facing
	var toward := CableDrape.skeleton(from, from_dir, to, to_dir, corners, floor_a, floor_b, radius, false,
		straight_a, straight_b)
	if clearance.hits(CableDrape.curve(toward), ctx).size() < clearance.hits(CableDrape.curve(facing), ctx).size():
		return toward
	return facing
...
## ---- sleeves -----------------------------------------------------------------
## A sleeve is a standalone run (`run_sleeve`) laid on the floor: a
## flexible braided sleeve that loose cables are threaded through. A
## cable in a sleeve runs from its terminal to the nearer end of the
## sleeve, through it, and out of the other end to its far terminal,
## its two tails laid on the floor like any loose cable; inside, it
## lies along the sleeve's own centre, hidden. The sleeve's size
## follows how many cables it carries.

var _cable_carriers: Dictionary = {}   # "src.port>dst.port" -> the sleeve a cable runs through


func _cable_key(visual: Dictionary) -> String:
	return "%s.%s>%s.%s" % [visual["a"], visual["a_port"], visual["b"], visual["b_port"]]


func _visual_by_key(key: String) -> Dictionary:
	for visual in _wire_visuals:
		if visual["node"] != null and _cable_key(visual) == key:
			return visual
	return {}


## The sleeve a cable runs through, or "".
func cable_sleeve(view: PipeView) -> String:
	for visual in _wire_visuals:
		if visual["node"] == view:
			return str(_cable_carriers.get(_cable_key(visual), ""))
	return ""


## A braided sleeve over n cables of 8 mm: the bundle packs at about
## seven tenths, and the braid adds a little.
func _sleeve_radius(name_: String) -> float:
	var count := 0
	for key: String in _cable_carriers:
		if str(_cable_carriers[key]) == name_:
			count += 1
	return maxf(StructureFactory.RUNS["run_sleeve"]["radius"],
		CABLE_RADIUS * sqrt(count / 0.7) + 0.002)


## The points a sleeve's centre passes through: its floor points,
## raised by its radius.
...
func set_sleeve_look(name_: String, clear: bool, label_: String) -> void:
	if not runs.has(name_):
		return
	checkpoint()
	var entry: Dictionary = runs[name_]
	entry["clear"] = clear
...
## A clear sleeve's braid: tinted glass, the cables inside seen through it.
const SLEEVE_CLEAR := Color(0.80, 0.86, 0.90, 0.22)


## The sleeve, tray or conduit a view draws, or "": the laid runs the
## player reshapes corner by corner like a line.
func _placed_of_view(view: PipeView) -> String:
	if view == null or not view.has_meta("editable_run"):
		return ""
	var name_ := str(view.get_meta("editable_run"))
	return name_ if runs.has(name_) and (runs[name_] as Dictionary)["node"] == view else ""


## A laid run's own points, as its handles stand: a sleeve's centre,
## a tray's or a conduit's points as laid.
func _run_corners(name_: String) -> Array:
	if (runs[name_] as Dictionary)["kind"] == "run_sleeve":
		return _sleeve_corners(name_)
	return ((runs[name_] as Dictionary)["points"] as Array).duplicate()
...
func _sleeved_route(src_name: String, src_port: String, dst_name: String, dst_port: String,
	...
	# Out of a conduit's mouth a cable runs on straight a little before it
	# falls away: it bends as a cable does, not at the lip.
	var mouth := CONDUIT_MOUTH if conduit else CableDrape.GLAND
	var skeleton := _clear_drape(from, from_dir, end_a, out_a, corners_a, floor_a, _carrier_floor(sleeve, end_a),
		radius, [src_name, ""], CableDrape.GLAND, mouth)
	...
	skeleton.append_array(_clear_drape(end_b, out_b, to, to_dir, corners_b, _carrier_floor(sleeve, end_b), floor_b,
		radius, ["", dst_name], mouth, CableDrape.GLAND))
...
const CONDUIT_MOUTH := 0.25   # a cable's straight out of a conduit's end
...
## it: a sleeve laid in the same frame as the slab it is on found the
## ground below.
func _settle_sleeves() -> void:
	for name_: String in runs:
		var entry: Dictionary = runs[name_]
		if entry["kind"] != "run_sleeve":
			continue
		var settled: Array = []
		var moved := false
		for p: Vector3 in entry["points"]:
			var y := _floor_y(p)
			var at := Vector3(p.x, y, p.z) if not is_nan(y) else p
			moved = moved or at.distance_to(p) > 0.001
			settled.append(at)
		if moved:
			entry["points"] = settled
			_resize_sleeve(name_)


## The level direction from a to b, or none.
static func _plan_dir(a: Vector3, b: Vector3) -> Vector3:
...
func toggle_sleeve(view: PipeView) -> Dictionary:
	...
	for candidate in _wire_visuals:
		if candidate["node"] == view:
			visual = candidate
	if visual.is_empty() or view.style() != "cable":
		return {"error": "only a loose cable goes in a sleeve"}
	var key := _cable_key(visual)
	if _cable_carriers.has(key):
		var old := str(_cable_carriers[key])
		checkpoint()
		_cable_carriers.erase(key)
		...
		_schedule_revalidate()
		return {"error": "", "sleeve": old, "into": false}
	var from := _marker_pos(str(visual["a"]), str(visual["a_port"]))
	var to := _marker_pos(str(visual["b"]), str(visual["b_port"]))
	var best := ""
	var best_reach := INF
	for name_: String in runs:
		var entry: Dictionary = runs[name_]
		...
		if entry["kind"] == "run_sleeve" or _is_conduit(name_):
			...
			reach = minf(maxf(_plan_distance(from, e0), _plan_distance(to, e1)),
				maxf(_plan_distance(from, e1), _plan_distance(to, e0)))
		elif _is_tray(name_):
			# A tray is joined anywhere along it.
			var tray := PipeRoute.lay(entry["points"])
			reach = maxf(from.distance_to(_on_path(tray, from)["point"]), to.distance_to(_on_path(tray, to)["point"]))
		if reach < best_reach:
			best_reach = reach
			best = name_
	if best == "":
...
const SLEEVE_REACH := 8.0


## ---- cables in a tray ----------------------------------------------------------
## A tray (`run_tray`) keeps the pipe rules: laid on its supports, never
## through anything. A cable threaded into it climbs in at the point of
## the tray nearest its source terminal, lies on the tray's floor in a
## place of its own across the width, and climbs out at the point
## nearest its destination. Its tails hang straight between terminal
## and tray.

const TRAY_PITCH := 0.03      # cable to cable across the tray
const TRAY_OVER := 0.14       # how far over the tray's floor a cable crosses its rail


func _is_tray(name_: String) -> bool:
	return runs.has(name_) and str(StructureFactory.RUNS[(runs[name_] as Dictionary)["kind"]]["style"]) == "tray"


## The nearest point of a laid path to p: {point, seg, along}, `along`
## the metres from the path's start.
static func _on_path(path: Array[Vector3], p: Vector3) -> Dictionary:
	var best := {"point": path[0], "seg": 0, "along": 0.0}
	var best_d := INF
	var walked := 0.0
	for i in path.size() - 1:
		...
		if d < best_d:
			best_d = d
			best = {"point": q, "seg": i, "along": walked + path[i].distance_to(q)}
		walked += path[i].distance_to(path[i + 1])
	return best
...
func _carrier_inside(carrier: String, key: String, from: Vector3, to: Vector3, radius: float) -> Array[Vector3]:
	...
	var inside: Array[Vector3] = [enter["point"]]
	if float(enter["along"]) <= float(leave["along"]):
		for i in range(int(enter["seg"]) + 1, int(leave["seg"]) + 1):
...
func _path_collision(path: Array[Vector3], radius: float, a: String, a_port: String,
	...
	for name_: String in runs:
		...
		if other.style() == "tray" or other.style() == "cable":
			continue
...
func snapshot() -> Dictionary:
	...
	for visual in _wire_visuals:
		...
		if visual["node"] != null and _cable_carriers.has(_cable_key(visual)):
			wire_entry["sleeve"] = _cable_carriers[_cable_key(visual)]
		var wire := sim.find_wire(sim.get_component(str(visual["a"])), str(visual["a_port"]),
			sim.get_component(str(visual["b"])), str(visual["b_port"]))
...
func restore(payload: Dictionary) -> bool:
	...
	for wire_entry: Dictionary in payload["wires"]:
		if str(wire_entry.get("sleeve", "")) != "":
			_cable_carriers["%s.%s>%s.%s" % [wire_entry["src"], wire_entry["src_port"], wire_entry["dst"],
				wire_entry["dst_port"]]] = str(wire_entry["sleeve"])
	for entry: Dictionary in payload.get("runs", []):
		...
		if bool(entry.get("clear", false)):
			set_sleeve_look(entry["name"], true, str(entry.get("label", "")))
		if str(entry.get("color", "")) != "":
