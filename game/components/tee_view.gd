...
func setup(tee_: SimTee) -> void:
	...
	var run_x := ViewUtil.cylinder(self, 0.075, 0.5, Vector3(0, LINE_Y, 0), steel)
	run_x.rotation_degrees = Vector3(0, 0, 90)
	var run_z := ViewUtil.cylinder(self, 0.075, 0.5, Vector3(0, LINE_Y, 0), steel)
	run_z.rotation_degrees = Vector3(90, 0, 0)
	ViewUtil.cylinder(self, 0.10, 0.18, Vector3(0, LINE_Y, 0), steel)
	# Flanges at the four faces.
	for offset: Vector3 in [Vector3(0.25, 0, 0), Vector3(-0.25, 0, 0)]:
		var flange := ViewUtil.cylinder(self, 0.13, 0.04, Vector3(0, LINE_Y, 0) + offset, steel)
		flange.rotation_degrees = Vector3(0, 0, 90)
	for offset: Vector3 in [Vector3(0, 0, 0.25), Vector3(0, 0, -0.25)]:
		var flange := ViewUtil.cylinder(self, 0.13, 0.04, Vector3(0, LINE_Y, 0) + offset, steel)
